...
for delta_y in np.logspace(2, 3.1, 20):   # is gaan van 10 tot 10000 in 100 logspace verdeelde stappen
    delta_y = int(delta_y)
    ...
    for Teller in range(0,20000):     # aantal keer dat m bepaald wordt op delta y
        ...
        for Stappen in range(0,delta_y):    # Massa bepalen
            Mass = Mass + Combined_rho[x_coord, (y_coord + Stappen)%(128*len(Timestep_index))]
...
plt.plot(delta_y_array, H_array, color="b", label="no rotation")
plt.xlabel('Delta (cells)')
plt.ylabel('H')
plt.legend(loc="center right")
plt.savefig('H_no_rot', bbox_inches='tight')
plt.figure()
plt.plot(delta_y_array, t0_array, color="b", label="no rotation")
plt.plot(delta_y_array, fit_function(delta_y_array, popt[0]), color="g", label="fitted function (14)")
plt.xlabel('Delta (cells)')
plt.ylabel("to")
plt.legend(loc="lower right")
plt.savefig('t0_no_rot', bbox_inches='tight')
